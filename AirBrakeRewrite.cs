...
namespace DvMod.AirBrake
{
    internal static class AirSystem
    {
        ...
        public static float Equalize(float dt, ref float p1, ref float p2, float v1, float v2, float multiplier = 1f, float speed = 1f)
        {
            var before = p1;
        ...
        public static float OneWayFlow(float dt, ref float fromPressure, ref float toPressure, float fromVolume, float toVolume, float speed = 1f)
        {
            if (fromPressure <= toPressure)
                return 0f;
            return Equalize(dt, ref toPressure, ref fromPressure, toVolume, fromVolume, speed);
        }

        public static float Vent(float dt, ref float pressure, float volume, float speed = 1f)
        {
            float atmosphere = 0f;
            return OneWayFlow(dt, ref pressure, ref atmosphere, volume, BrakeSystemConsts.ATMOSPHERE_VOLUME, speed);
        }
    }
        ...
        static class UpdatePatch
        {
            ...
            */
            ...
            private static void ApplyBrakingForce(BrakeSystem car)
            {
                ...
                var cylinderBrakingFactor = Mathf.InverseLerp(THRESHOLD_PRESSURE, MAX_CYLINDER_PRESSURE, state.cylinderPressure);
            ...
            private static void DebugLog(BrakeSystem car, string msg)
            {
